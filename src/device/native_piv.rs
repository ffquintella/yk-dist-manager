...
impl PivWriter for NativePiv {
    ...
    fn create_csr(
        ...
        // Refused rather than approximated. The whole reason this step is native
        // is the `rfc822Name` SAN; a request without it would come back from the
        // CA as a certificate that does not do its job, and silently.
        //
        // The `yubikey` crate offers self-signed certificates and raw signing,
        // not PKCS#10, so the request has to be assembled and signed through the
        // card. Tracked in `features/step-piv-signing-certificate.md`.
        Err(WriteError::Unsupported {
            operation: "piv.create_csr",
            reason: format!(
                "building a PKCS#10 request with an rfc822Name SAN ({san_email}) is not \
                 implemented yet — the crate has no CSR builder, so the request must be \
                 assembled and signed through the card"
            ),
        })
    }
