...
| 3 | Applicability rules per key (firmware, applications present) | 1 | **Done** | [`template::applicability`](../src/template/applicability.rs) — a firmware floor, a firmware ceiling and the applications a procedure needs, checked by the pre-flight as a **refusal about the run** rather than as per-step skips: a wrong step skips, a wrong procedure that starts leaves the register claiming a key had something applied to it that it was never meant for. A rule that could not be checked is a *warning*, never a refusal — a key entered by barcode has no firmware on record, and refusing it would refuse one of the three supported ways of entering a serial. Deliberately three fields: each is answerable from a read this tool already makes, and an unevaluable rule turns into a warning nobody can clear |
...
| 7 | Per-step retry / continue-on-failure policy | 1 | **Done** | `TemplateStep::attempts`, bounded at 5 and clamped on read, with `required` still the continue-on-failure half. **Which failures spend the budget is decided by the error, not the procedure** — [`WriteError::is_worth_retrying`](../src/device/write.rs): a dropped frame yes, a rejected secret no, because three attempts at a mistyped PIV PIN is how a PIN gets blocked. Audited `bootstrap.step.retried` per attempt and named in the step's detail, so a flaky reader is visible. The signed canonical bytes cover it — see the second format tag below |
