...
| 6 | Warn clearly when a default is still present | **Done** | `PivState::factory_defaults` and the *Configuration* column on **Attached now**. The pre-flight's warning is seen once, by the operator about to fix it; a fleet audit a year later needs it on the key's own row. It rests on the three `Option<bool>` metadata answers rather than on a boolean, because **unknown must not be badged as a default** — a badge that accused a correctly configured key of holding a factory PIN because a reader was busy is one an operator learns to ignore, after which it is worth less than none |
