...
- **246 tests** pass on the default features (`cargo test`); 245 with
...
- `cargo check --no-default-features` is part of the pre-commit sweep, because the
  ...
  (`make check-all`).
- Core line coverage **86.86%** (region 84.20%), above the 80% floor.
- Unit suites: `unit_domain.rs`, `unit_template.rs`, `unit_audit.rs`,
  `unit_ykman_parse.rs`, `unit_store.rs` (23), `unit_device_backends.rs`,
...
- Behaviour suites: `behaviour_distribution.rs` (7 scenarios),
  `behaviour_bootstrap.rs` (10), `behaviour_storage.rs` (10),
