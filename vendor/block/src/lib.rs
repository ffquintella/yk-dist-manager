...
// PATCHED (features/packaging-and-release.md phase 0b). Upstream declares this
// as `enum Class { }` — an uninhabited type — and then declares an `extern`
// static of it below. rustc's `uninhabited_static` lint reports that as a future
// incompatibility (rust-lang/rust#74840): an uninhabited static cannot be
// initialised and any *access* to it would be an immediate error.
//
// Nothing here ever accesses it. `Class` exists only so that `&_NSConcreteStackBlock`
// can be taken as an opaque `*const Class` and handed to the blocks runtime, and
// this declaration says exactly that instead: a zero-sized `#[repr(C)]` type,
// which is inhabited, has the same size and alignment behaviour at a pointer, and
// cannot be constructed by accident because its only field is private.
#[repr(C)]
struct Class {
    _opaque: [u8; 0],
}
...
// The upstream test module is not carried over: it calls into an
// `objc_test_utils` helper crate that the published package excludes, so it
// cannot be built from a copy of the crate. Nothing in this shim changes what
// those tests cover — see `vendor/block/README.md`.
