...
452 tests pass (plus 2 read-only hardware tests), with 87.78% line coverage of the
headless core. The register can also be opened from the unit's **SMB share**, connected
...
| `[/]` | Testing strategy | [spec](features/testing-strategy.md) — 452 tests across unit + behaviour suites, a mock device backend and a mock share connector, recorded fixtures, ignored hardware tests; 87.78% core line coverage. The gate is not yet enforced in CI. |
